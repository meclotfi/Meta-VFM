"""SiPakMed cytology dataloader with stratified train/val/test splits."""
...
def _stratified_split(
    ...
    if val_ratio + test_ratio >= 1:
        raise ValueError("val_ratio + test_ratio must sum to less than 1.")
    ...
    for class_idx in range(num_classes):
        ...
        if val_size + test_size >= total:
            raise ValueError(
                f"Not enough samples in class index {class_idx} for requested splits."
            )
...
def get_data_loaders_sipakmed(config):
    """Return train/val/test dataloaders for the SiPakMed dataset."""
    root = Path(config.data_root).expanduser() / "SipakMed"
    if not root.exists():
        raise FileNotFoundError(f"SiPakMed root not found: {root}")

    train_transform, eval_transform = get_transforms()

    raw_samples = _gather_samples(root)
    ...
    train_dataset = SipakMedDataset(train_samples, transform=train_transform)
    val_dataset = SipakMedDataset(val_samples, transform=eval_transform) if val_samples else None
    test_dataset = SipakMedDataset(test_samples, transform=eval_transform) if test_samples else None
...
if __name__ == "__main__":
    ...
    get_data_loaders_sipakmed(config)
